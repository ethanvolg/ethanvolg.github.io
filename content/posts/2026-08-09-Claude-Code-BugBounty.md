...
많은 사람들이 AI에게 "버그 찾아줘"라고 말한다. 그러면 AI는 이것저것 뱉어내지만, 실제로 쓸 수 있는 결과는 거의 없다. AI의 진짜 쓸모는 사람이 하기엔 너무 반복적이고 시간이 오래 걸리는 작업을 빠르게 처리해 주는 것이다.

### 기존 도구가 못 하는 것

Amass, Subfinder, ffuf 같은 기존 해킹 도구들은 {{< term label="서버 주소나 경로" title="도메인과 경로란?" >}}웹 주소를 보면 `https://app.target.com/admin/users` 이런 식으로 생겼습니다. 여기서 `app.target.com`이 도메인이고, `/admin/users`가 경로입니다.{{< /term >}}를 자동으로 찾아주는 데 강하다. 그런데 요즘 웹 서비스들은 다르다. React, Vue, Next.js 같은 기술로 만들어진 서비스는 경로를 서버가 직접 알려주지 않는다. 대신 자바스크립트(JS) 파일 안에 경로를 넣어서 브라우저가 알아서 처리하게 한다.

기존 도구로는 이 경로들을 찾을 수 없다. 서버한테 물어봐도 모른다고 하거든. 정보가 JS 파일 안에 숨어 있으니까.

Claude Code는 이 JS 파일을 직접 읽고 분석할 수 있다. 숨겨진 경로, 사용하지 않는 API 주소, 관리자 기능처럼 보이는 코드 등을 찾아낸다. 기존 도구가 놓치는 부분이다.
...
HackerOne에는 과거에 신고된 취약점 리포트들이 공개되어 있다. 이것을 모아서 읽다 보면 패턴이 보인다. "이런 상황에서 이런 취약점이 자주 나오더라"는 식으로.

이때 중요한 건 걸러내기다. 심각하지 않은 취약점 리포트는 제외한다. 개인정보 유출, 원격 코드 실행, {{< term label="SSRF" title="SSRF란?" >}}서버가 내 대신 다른 곳에 요청을 보내도록 만드는 공격입니다. 서버 내부 시스템에 접근할 수 있게 됩니다.{{< /term >}}, {{< term label="IDOR" title="IDOR란?" >}}다른 사람의 데이터에 내 계정으로 접근할 수 있는 취약점입니다. 예를 들어 URL에서 숫자 하나만 바꿔도 다른 사람 정보가 보이는 경우입니다.{{< /term >}}, {{< term label="GraphQL 권한 우회" title="GraphQL 권한 우회란?" >}}GraphQL은 데이터를 요청하는 방식 중 하나인데, 권한 확인이 제대로 안 되어 있으면 내가 접근하면 안 되는 데이터를 가져올 수 있습니다.{{< /term >}}처럼 위험도가 높은 것만 추린다.

### 스킬 파일 만들기

추려낸 패턴을 마크다운 파일로 저장한다. AI가 분석을 시작하기 전에 이 파일을 먼저 읽도록 지시하면, 내가 원하는 방향으로 분석해 준다.
...
타깃 사이트마다 폴더를 따로 만들고, 그 안에 {{< term label=".claude.md" title=".claude.md란?" >}}Claude Code는 프로젝트 폴더에 `.claude.md` 파일이 있으면 그것을 자동으로 읽습니다. 마치 새 직원한테 "우리 회사 규칙입니다"라고 문서를 주는 것과 같습니다. AI가 어떤 역할을 해야 하는지, 무엇을 찾아야 하는지 미리 알 수 있습니다.{{< /term >}}<!-- --> 파일을 쓴다. Claude Code는 폴더에 진입할 때 이 파일을 자동으로 읽는다. AI가 어떤 역할을 해야 하는지, 무엇을 찾아야 하는지 매번 말하지 않아도 된다.
...
> 💰 <strong>비용 참고:</strong> Claude Pro 플랜(월 20달러)은 JS 파일을 많이 분석하다 보면 금방 한도에 걸린다. Archangel은 Claude Max 플랜(월 100달러)을 썼다. 포상금 한 건이면 구독료를 충분히 뽑을 수 있는 구조다.
...
타깃 사이트의 JS 파일을 먼저 내려받는다. 실제 서비스의 JS 파일 이름에는 `main.a3f92bc.js` 같이 무작위 문자열이 붙어 있어서 URL을 직접 추측하기 어렵다.

두 가지 방법이 있다.

- <strong>브라우저로 직접 확인하기 (도구 설치 불필요):</strong> 타깃 사이트에서 F12를 눌러 개발자 도구를 열고, Network 탭에서 JS 파일만 필터링한다. 로드된 파일 목록이 보이면 URL을 복사해서 다운로드하면 된다.
- <strong>자동 수집 도구 쓰기:</strong> `katana`, `gau`, `hakrawler` 같은 도구를 쓰면 사이트를 돌아다니면서 JS 파일 주소를 자동으로 긁어온다.

### Step 2. Claude Code로 분석하기

JS 파일을 `./js_files/` 폴더에 넣은 다음, Claude Code에게 분석을 시킨다. 스킬 파일을 먼저 읽으라고 지시하는 것이 핵심이다.
...
분석이 오래 걸릴 것 같으면 끝낼 조건을 명확하게 준다. 그러면 Claude Code가 중간에 멈추지 않고 끝까지 작업한다.
...
{{< term label="GraphQL" title="GraphQL이란?" >}}데이터를 주고받는 방식 중 하나입니다. REST API와 달리, 클라이언트가 원하는 데이터를 직접 골라서 요청할 수 있습니다. 설정이 잘못되면 접근하면 안 되는 데이터까지 가져올 수 있게 됩니다.{{< /term >}}을 쓰는 서비스라면 Claude Code에게 스키마를 파악하게 한다.
...
Claude Code가 세션 정보를 찾지 못할 때는, {{< term label="Burp Suite나 Caido" title="Burp Suite / Caido란?" >}}내 브라우저와 서버 사이에서 오가는 요청을 중간에 가로채서 볼 수 있는 도구입니다. 어떤 데이터가 어떻게 오가는지 확인하고, 값을 바꿔서 다시 보낼 수 있습니다.{{< /term >}}에서 캡처한 실제 HTTP 요청을 직접 붙여넣는다.
...
- <strong>권한 재확인:</strong> 다른 테스트 계정의 쿠키를 직접 넣어보고, 실제로 다른 사람 데이터가 보이는지 확인한다.
- <strong>데이터 보호:</strong> DELETE나 UPDATE 요청을 테스트할 때는, 실제 데이터가 지워지거나 바뀌지 않도록 범위를 먼저 확인한다.
- <strong>실제 피해 증명:</strong> 단순히 에러 메시지가 다르게 나오는 게 아니라, 이름·이메일·전화번호 같은 실제 개인정보가 응답 데이터에 담겨서 오는지 확인해야 진짜 취약점이다.
...
AI는 해커를 대신하는 도구가 아니다. 분석 속도를 빠르게 높여주는 파트너다. 나는 방향을 잡고, AI는 반복 작업을 처리한다. 이 역할 분담이 맞아야 제대로 쓴다.
